...
def load_song_annot(songfile):
    ...
    if dirname == '':
        annot_file = glob.glob('../March_07_nFB_morning_annot/'+songfile[0:15]+'_annot.txt')
    else:
        annot_file = glob.glob(os.path.join(dirname, '../March_07_nFB_morning_annot/'+songfile[0:15]+'_annot.txt'))
    if len(annot_file) < 1:
